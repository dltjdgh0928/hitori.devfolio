    ...
    <section id="contact" className="py-24 px-6 bg-background">
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <div 
            ref={introRef}
            className={`flex flex-col lg:flex-row items-center gap-8 max-w-4xl mx-auto scroll-animate ${introVisible ? 'animate' : ''}`}
        ...
        <div 
          ...
          className={`grid md:grid-cols-2 gap-8 mb-16 max-w-2xl mx-auto scroll-animate ${contactMethodsVisible ? 'animate' : ''}`}
          style={{ transitionDelay: '0.4s' }}
        >
          {contactMethods.map((method, index) => (
            <Card 
              key={index}
              className={`card-gradient border-0 transition-all duration-500 group cursor-pointer ${getColorClass(method.color)} hover:scale-105 hover:shadow-2xl scroll-animate-stagger ${contactMethodsVisible ? 'animate' : ''}`}
        ...
        <div 
          ref={socialRef}
          className={`text-center scroll-animate ${socialVisible ? 'animate' : ''}`}
